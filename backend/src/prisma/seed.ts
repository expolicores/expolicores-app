...
import { PrismaClient, Role, OrderStatus, Prisma, Product } from '@prisma/client';
...
async function main() {
  ...
  const baseProducts: Prisma.ProductCreateInput[] = [
  ...
  if (ADD_DEMO_ITEMS) {
    ...
    const extra: Prisma.ProductCreateManyInput[] = [];
    for (let i = 1; i <= 60; i++) {
  ...
  // ============== Órdenes de ejemplo ==============
  const pfind = (name: string) => createdBase.find(x => x.name.includes(name))!;
