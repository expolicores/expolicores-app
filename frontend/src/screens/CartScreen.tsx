...
export default function CartScreen() {
  const navigation = useNavigation<any>(); // 👈 NUEVO
  ...
  const renderItem = ({ item }: any) => (
    <View className="flex-row items-center bg-white rounded-2xl p-3 mb-3 shadow">
      ...
      <View className="flex-1">
        ...
        <Text className="text-emerald-700 font-bold mt-1">
          ${(item.price * item.qty).toLocaleString('es-CO')}
        </Text>
        <View className="flex-row items-center mt-2">
          <Pressable
            onPress={() => setQty(item.productId, Math.max(1, item.qty - 1))}
            className="bg-slate-200 px-3 py-1 rounded-xl mr-2"
          >
            <Text>-</Text>
          </Pressable>
          <Text className="mx-1 font-semibold">{item.qty}</Text>
          <Pressable
            onPress={() => setQty(item.productId, Math.min(item.qty + 1, item.stock))}
            className="bg-slate-200 px-3 py-1 rounded-xl ml-2"
          >
            <Text>+</Text>
          </Pressable>
        </View>
      </View>
      <Pressable onPress={() => remove(item.productId)} className="ml-3">
